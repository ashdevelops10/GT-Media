...
function VideoCard({ collab, index }: { collab: typeof collabs[0]; index: number }) {
 ...
 className={`relative flex-shrink-0 snap-center overflow-hidden group cursor-pointer ${collab.format ==="reel"?"w-[280px] aspect-[9/16]":"w-[400px] aspect-[16/9]"}`}
 >
 ...
 className="absolute inset-0 w-full h-full object-cover" src={(collab as any).videoSrc}
 ...
 <button
 onClick={toggle}
 className="absolute inset-0 flex items-center justify-center z-20" aria-label={playing ?"Pause":"Play"}
 >
 <div className={`w-14 h-14 bg-black/50 backdrop-blur-md flex items-center justify-center border border-white/30 transition-opacity duration-300 ${playing ?"opacity-0 group-hover:opacity-100":"opacity-100"}`}>
 {playing ? (
