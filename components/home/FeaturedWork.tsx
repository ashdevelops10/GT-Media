...
function ProjectCard({ project, index }: { project: typeof featuredProjects[0]; index: number }) {
 ...
 className={`${project.span} ${project.tall ?"aspect-[3/4]":"aspect-[16/9]"} relative group overflow-hidden bg-black border border-white/[0.06] hover:border-burgundy/40 transition-all duration-300`}
 ...
 loop
 playsInline
 preload="metadata" className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-105 z-10" />
 ) : (
 <div
 className="absolute inset-0 bg-black transition-transform duration-700 group-hover:scale-105" style={{ backgroundImage: `url(${project.image})`, backgroundSize: 'cover', backgroundPosition: 'center' }}
 />
 )}
...
export function FeaturedWork() {
 ...
 <Link
 href="/work" className="group inline-flex items-center gap-2 px-6 py-3 border border-white/15 text-white/70 text-xs font-bold uppercase tracking-wider hover:border-burgundy/40 hover:text-burgundy transition-all duration-300" >
