...
function BezierArc({ start, end, delay = 0.5 }: { start: { x: number; y: number }; end: { x: number; y: number }; delay?: number }) {
 ...
 <motion.path
 d={path}
 fill="none" stroke="url(#arcGradient)" strokeWidth="1" strokeDasharray="1000" strokeDashoffset="1000" animate={{ strokeDashoffset: 0 }}
...
export function InternationalCollabs() {
 ...
 <div
 ref={ref}
 className="relative w-full aspect-[16/10] md:aspect-[3/1] bg-black/30 border border-white/5 overflow-hidden group shadow-2xl" >
 {/* World Map Backdrop */}
 <svg
 ...
 {/* Hub Pin (India) */}
 <div
 className="absolute z-20" style={{ left: `${HUB_LOCATION.x}%`, top: `${HUB_LOCATION.y}%` }}
 >
 <div className="relative -translate-x-1/2 -translate-y-1/2">
 <div className="absolute -inset-8 rounded-full bg-burgundy/10 animate-pulse"/>
 <div className="absolute -inset-4 rounded-full bg-burgundy/20 animate-ping"/>
 <div className="w-3 h-3 rounded-full bg-white border-2 border-burgundy shadow-[0_0_15px_rgba(229,56,59,0.8)]"/>
 ...
 className="absolute z-30 group/pin cursor-pointer" style={{ left: `${loc.coords.x}%`, top: `${loc.coords.y}%` }}
 onMouseEnter={() => setActiveLocation(loc.id)}
 onMouseLeave={() => setActiveLocation(null)}
 >
 <div className="relative -translate-x-1/2 -translate-y-1/2">
 {/* Secondary pulse */}
 <div className="absolute -inset-4 rounded-full bg-burgundy/30 opacity-0 group-hover/pin:opacity-100 group-hover/pin:animate-ping transition-opacity"/>
 <div className="relative w-4 h-4 rounded-full bg-burgundy/80 hover:bg-burgundy border-2 border-white/50 shadow-lg transition-colors"/>
 ...
 className="absolute bottom-6 left-1/2 -translate-x-1/2 w-52 bg-black/80 backdrop-blur-xl border border-white/10 overflow-hidden shadow-2xl z-50 pointer-events-none" >
 ...
 <div className="absolute top-2 right-2 px-1.5 py-0.5 bg-burgundy/90 text-[8px] font-bold text-white uppercase tracking-wider">
 Live Case
 </div>
 </div>
 <div className="p-4">
