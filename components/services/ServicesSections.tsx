...
function ServiceModule({ service, index }: { service: typeof services[0]; index: number }) {
 ...
 className="text-xs uppercase tracking-widest text-burgundy mb-4" >
 0{index + 1}
 </motion.p>
 <motion.h2
 initial={{ opacity: 0, y: 20 }}
 ...
 className="aspect-[16/9] bg-white/20/10 mb-8 relative overflow-hidden" >
 <Image
 src={service.image}
 alt={service.title}
 fill
 className="object-cover" sizes="(max-width: 768px) 100vw, 60vw" />
 </motion.div>
 ...
 >
 <p className="text-sm text-white/50/70 mb-2">PROCESS</p>
 <p className="text-white/50">{service.process}</p>
 </motion.div>
 </div>
 }
 right={
 ...
 <div className="mb-8">
 <p className="text-sm text-white/50/70 mb-4">DELIVERABLES</p>
 <ul className="space-y-3">
 {service.deliverables.map((item) => (
 <li key={item} className="text-white/50 flex items-start">
 <span className="mr-3 mt-1.5 w-1.5 h-1.5 rounded-full bg-burgundy flex-shrink-0"/>
 {item}
 </li>
 ))}
 </ul>
 ...
 <div className="border-t border-white/20/20 pt-6">
 <p className="text-sm text-white/50/70 mb-2">OUTCOMES</p>
 <p className="text-white/50">{service.outcomes}</p>
 </div>
 </motion.div>
 }
 />
