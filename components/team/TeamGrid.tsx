...
function ProfileIllustration({ initials, index }: { initials: string; index: number }) {
 ...
 cx="200"cy="200"r="140" fill={`url(#grad-${index})`}
 />
 <motion.circle
 initial={{ pathLength: 0, opacity: 0 }}
 whileInView={{ pathLength: 1, opacity: 1 }}
 transition={{ duration: 1.5, delay: 0.5 }}
 cx="200"cy="200"r="160" fill="none" stroke={isEven ?"#b91c1c":"#78716c"}
 strokeOpacity="0.4" strokeWidth="0.75" strokeDasharray="8 6" />
 <motion.path
 initial={{ opacity: 0 }}
 ...
 x1="0"y1="400"x2="400"y2="0" stroke="white"strokeOpacity="0.06"strokeWidth="1" />
 </svg>
 <motion.span
...
export function TeamGrid() {
 ...
 <div className="relative overflow-hidden border border-white/[0.1] bg-white/[0.04] backdrop-blur-sm hover:border-burgundy/30 transition-all duration-700">
 <div className="aspect-[4/3] relative bg-neutral-950">
 <ProfileIllustration initials={member.initials} index={index} />
 <div className="absolute top-5 left-5">
 <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white/[0.1] backdrop-blur-md border border-white/[0.12] text-[10px] uppercase tracking-[0.15em] text-white/70 font-medium">
 ...
 <span
 key={s}
 className="px-3 py-1 bg-white/[0.07] border border-white/[0.1] text-[11px] text-white/55 tracking-wide" >
 {s}
 </span>
 ))}
 </div>
